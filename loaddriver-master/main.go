...
func main() {
	...
	r.PathPrefix("/intensities/").Handler(custom.NoCache(http.StripPrefix("/intensities/", http.FileServer(http.Dir("./intensities/")))))
	r.PathPrefix("/scripts/").Handler(custom.NoCache(http.StripPrefix("/scripts/", http.FileServer(http.Dir("./scripts/")))))
	go hub.Run()
	go jobRunner.Start()
