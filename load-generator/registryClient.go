...
func startRegistryHeartbeat() {
	...
	for c := time.Tick(30 * time.Second); ; <-c {
		...
		if err != nil {
			log.Warnf("Registry under http://%s:%d is not responding, trying again in 1 minute", registryHost, registryPort)
			continue
		}
		if res.StatusCode != http.StatusOK {
			log.Warnf("Registry under http://%s:%d rejected registration, trying again in 1 minute", registryHost, registryPort)
			continue
		}
	}
}
